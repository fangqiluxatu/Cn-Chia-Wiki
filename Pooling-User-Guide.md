...
### 远程收割
远程收割耕种跟之前一样，你可以直接使用其他的机器上使用 `-f` 以及 `-c` 参数来进行农田的开垦工作，然后开始收割，不需要使用账号私钥。但你内网的农民主机需要使用私钥登录，以便节点、钱包及农田产权证的状态同步到最新。当农田产权证的难度越低，你的收割机向矿池提交的证明就会越频繁。你可以使用 `chia farm summary` 来查看内网中收割机正在耕种的农田总数。
