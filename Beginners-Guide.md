翻译自[2021年5月11日版本-78#](https://github.com/Chia-Network/chia-blockchain/wiki/Beginners-Guide/dfb2665c744a65f97f59296a24d66a9f6cafc5df)
***
...
开始的方式很简单，根据你的计算机系统来[安装相应版本的Chia软件](INSTALL.md)并打开GUI客户端。Windows版的在安装完后会自动启动，MAC版的需要从应用目录中打开软件。当软件启动后，会开始连接到网络并同步至最新区块。在本文撰写的时候，4~6个小时就可以完全同步至最新，但这个时间会随着区块网络的增长而变长。当你与区块上的每一个节点同步的时候，会下载全部的区块数据，里面包含了所有已经处理验证过的信息，Chia区块网络的数据将会被保存在本地。每一个安装运行Chia的计算机都将会有一份整个区块网络数据的副本。
